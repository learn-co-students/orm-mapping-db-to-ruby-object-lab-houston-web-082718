...
class Student
  ...
  def self.first_student_in_grade_10
    DB[:conn].execute("SELECT * FROM students WHERE grade = 10 ORDER BY id LIMIT 1").map do |row|
      self.new_from_db(row)
    end.first
  end

  def self.first_X_students_in_grade_10(num)
    DB[:conn].execute("SELECT * FROM students WHERE grade = 10 ORDER BY id LIMIT ?", num).map do |row|
      self.new_from_db(row)
    end
  end
